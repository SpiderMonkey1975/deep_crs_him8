...
Usage: python3 crs_dcgan_small.py
'''
...
class DCGAN(object):
    def __init__(self, img_rows=200, img_cols=200, channel=3):
    ...
    def generator(self):
     ...
     #   self.G.add(Activation('relu'))
        ...
        self.G.add(Conv2D(128, 5, strides=1, padding='same'))
        self.G.add(BatchNormalization(momentum=0.9))
        self.G.add(Activation('relu'))
...
class CRS_DCGAN(object):
    def __init__(self):
        ...
        self.x_train = self.x_train[ 0:self.img_rows, 0:self.img_cols ]
    ...
    def train(self, batch_size=256):
        ...
        noise_input = noise_input[ :,0:self.img_rows,0:self.img_cols,: ]
    ...
    def plot_images(self, fake=True, samples=16):   
        ...
        if fake:
            ...
            noise = np.expand_dims( noise_input[ i,0:self.img_rows,0:self.img_cols,channel_no ], axis=3 )
